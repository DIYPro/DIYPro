...
public class StatsServiceImplements implements StatsService {
	...
	public void genFurnitureList(Model model, UserVO user) {
		...
		ArrayList<RankVO> rankList = new ArrayList<>();

		// 성별, 생일
		String[] arrBirth = user.getBirth().split("-");
		int birth = new Integer(arrBirth[0]);
		int lstBirth;//임시
		
		// 현재 날짜의 연도를 저장할 형식과, Calendar Instance
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy");
		Calendar c1 = Calendar.getInstance();

		// 현재 날짜의 연도(+형변환)
		String strYear = sdf.format(c1.getTime());
		int nYear = new Integer(strYear);

		// 나이
		int age = nYear - birth;
		int lstAge;//임시
		
		vo = dao.genFurnitureList(user);

		for (int i = 0; i < vo.size(); ++i) {
			arrBirth = vo.get(i).getBirthYear().split("-");
			lstBirth = new Integer(arrBirth[0]);
			lstAge = nYear - lstBirth;

			if (age > 19 && age < 30) // 20대 남자
			{
				if (lstAge > 19 && lstAge < 30) {
					rankList.add(vo.get(i));
				}
			} else if (age > 29 && age < 40) // 30대 남자
			{
				if (lstAge > 29 && lstAge < 40) {
					rankList.add(vo.get(i));
				}
			} else if (age > 39 && age < 50) // 40대 남자
			{
				if (lstAge > 39 && lstAge < 50) {
					rankList.add(vo.get(i));
				}
			} else if (age > 49 && age < 60) // 50대 남자
			{
				if (lstAge > 49 && lstAge < 60) {
					rankList.add(vo.get(i));
				}
			}
		}
		...
		for (int i = 0; i < 3; i++) {
			img.add(rankList.get(i).getImgPath());
		}
