...
<!-- SITE META -->
<title>HomeStyle | Responsive Furniture Website Template2</title>
...
<body style="overflow: visible;">
	...
	<script>
			...
			function init() {
				...
				//planeMaterial = new THREE.MeshBasicMaterial( {color: 0xEDEDED, side: THREE.DoubleSide} );
                /*var wallTexture = THREE.ImageUtils.loadTexture('../editor/textures/depositphotos_83939646-stock-illustration-wallpaper-texture.jpg');//, {}, function(){renderer.render(scene)});*/
                wallTexture = new THREE.ImageUtils.loadTexture(path+'pwall.jpg');
...
//     						addRow = '<tr><th>1</th><th>2</th><th>3</th></tr>';
    						...
    						$(result).each(
    							function(index, item){
    								...
    								addRow +='<td class="furImg"><img src="'+path+item.imgPath+'" style="width:70px;height:70px; margin:5px" onclick="UIcontrols.buildFurni('+'\''+obj+'\''+','+'\''+mtl+'\''+','+'\''+item.furnitureNum+'\')"></td>';
              ...
              //임시_식탁
                this.buildFurni = function(objPath,mtlPath,furnitureNum)
                {  
            	  	
                	$.ajax({
    					url : "/admin/file/furnitureRecord",
    					type : "post",
    					data : {
...
//             	  	console.log(mtlPath);
                ...
                {
                	if(wallTextureState == 1)
                	{
                		console.log(selectWall);
                		wallTexture = new THREE.ImageUtils.loadTexture(path+'wall3.jpg');
                		var mats = [];
...
// 							동근수정
        ...
        function createDoor(intersect)
        {
            ...
            mats.push(new THREE.MeshPhongMaterial({ map: wallTexture}));        
            mats.push(new THREE.MeshPhongMaterial({ map: wallTexture}));
             //wallMaterials = new THREE.MeshFaceMaterial(mats2);   
            
            resultObj.geometry.computeFaceNormals();
            resultObj.geometry.computeVertexNormals();
            resultObj.material = new THREE.MeshFaceMaterial(mats);
            resultObj.name = 'wall';
            scene.add(resultObj);            
		...
		<div id='cssmenu'>
			<ul>
				<li class='has-sub'><a href='#'><i class="fa fa-3x fa-list"
					...
					<ul>
						<li><a href='#' onclick="UIcontrols.saveScene()"><i
								class="fa fa-3x fa-floppy-o" aria-hidden="true"
								style="vertical-align: middle;"></i></a></li>
						<li><a href='#' onclick="UIcontrols.loadScene()"><i
								class="fa fa-3x fa-leanpub" aria-hidden="true"
								style="vertical-align: middle;"></i></a></li>
					</ul></li>
				<li><a href='#' onclick="UIcontrols.unDo()"><i
						class="fa fa-3x fa-undo" aria-hidden="true"
		...
		<div id='cssmenuVertic'>
			<ul>
				...
				<li><a href='#'><i class="fa fa-3x fa-delicious"
						style="vertical-align: middle;"></i></a>
					<ul>
						<li class='has-sub'><a href='#' onclick="UIcontrols.changeTexture1()">
						<img src="/admin/resources/file/wallPaperIcon.png" style="margin:5px;width:30px;height:30px;"></a>
						<li class='has-sub'><a href='#' onclick="UIcontrols.buildDoor()">
						<img src="/admin/resources/file/doorIcon.png" style="margin:5px;width:30px;height:30px;"></a> <!-- <ul> -->
						<li><a href='#' onclick="UIcontrols.buildWindow()">
						<img src="/admin/resources/file/windowIcon.png" style="margin:5px;width:30px;height:30px;"></a></li>
					</ul>
						<!-- </ul> --></li>
